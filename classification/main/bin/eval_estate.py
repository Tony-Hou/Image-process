...
import time
import os
from train_estate import get_split, load_batch
...
dataset_dir = '/home/houlinjie001/classifi/lianjia/'
...
def run():
	...
	y_true = tf.placeholder(tf.int32, shape=[None], name='y_true')
        ...
        with slim.arg_scope(inception_resnet_v2_arg_scope()):
            logits, end_points = inception_resnet_v2(x, num_classes = dataset.num_classes, is_training = False)
	...
	logging.info('the predictions:', end_points['Predictions'])
        ...
        def eval_step(sess, metrics_op, global_step, feed_dicts):
            '''
            Simply takes in a session, runs the metrics op and some logging information.
            '''
            start_time = time.time()
            _, global_step_count, accuracy_value = sess.run([metrics_op, global_step_op, accuracy], feed_dicts)
		...
		feed_dict={x: image, y_true: label}
                sess.run(sv.global_step, feed_dict)
                #print vital information every start of the epoch as always
                if step % num_batches_per_epoch == 0:
                    logging.info('Epoch: %s/%s', step / num_batches_per_epoch + 1, num_epochs)
                    logging.info('Current Streaming Accuracy: %.4f', sess.run(accuracy, feed_dict))
                    
                #Compute summaries every 10 steps and continue evaluating
                if step % 10 == 0:
                    eval_step(sess, metrics_op = metrics_op, global_step = sv.global_step, feed_dicts=feed_dict)
