...
def train():
    ...
    with tf.Graph().as_default() as graph:
        ...
        #placeholder method load dataset
        ...
        def train_step(sess, train_op, global_step, feed_dict_value):
            '''
            Simply runs a session for the three arguments provided and gives a logging on the time elapsed for each global step
            '''
            #Check the time for each sess run
            start_time = time.time()
            total_loss, global_step_count, _ = sess.run([train_op, global_step, metrics_op], feed_dict_value)
        ...
        with sv.managed_session(config=config) as sess:
            for step in xrange(num_steps_per_epoch * num_epochs):
                ...
                if step % num_batches_per_epoch == 0:
                    ...
                    logits_value, probabilities_value, predictions_value, labels_value = sess.run([logits, probabilities, predictions, labels], feed_dict)
                    print 'logits: \n', logits_value
                    print 'Probabilities: \n', probabilities_value
                    print 'predictions: \n', predictions_value
                    print 'Labels:\n:', labels_value
                ...
                if step % 10 == 0:
                    loss, _ = train_step(sess, train_op, sv.global_step, feed_dict)
                    summaries = sess.run(my_summary_op, feed_dict)
