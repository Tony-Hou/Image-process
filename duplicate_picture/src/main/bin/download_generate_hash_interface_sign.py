...
#         pass
#     if glob.glob('process.log.2018-08-20_20-?') == "":
#         pass
#     else:
#         dir = './' + date + '/'
#         shutil.move(glob.glob('process.log.2018-08-20_20-*')[0], dir)
#
#
# scheduler = GeventScheduler()
# # scheduler.add_job(log_job, 'cron', hour=15, minute=45)
...
# # FileHandler
# process_handler = logging.handlers.TimedRotatingFileHandler('process.log', when='M', interval=1)
# exception_handler = logging.handlers.TimedRotatingFileHandler('exception.log', when='M', interval=1)
# process_log.suffix = "%Y%m%d-%H%M.log"
# process_log.suffix = "%Y%m%d-%H%M.log"
...
def batch_image_process():
    ...
    try:
        ...
        process_log.info('REQUEST_ID:%s|client IP:%s|request method:%s|request URL:%s|request parameter:%s|HTTP_CODE:%d'
                         '|response time:%s|req_size:%d', request_id, request.remote_addr, request.method, request.path, data, 1,
                         (time.time() - start_time), len(ret_val))
